...
const globalConfig = JSON.parse(fs.readFileSync(path.join(__dirname, '../global-variables.json'), 'utf8'));

test('Dristi Kerala login and file a case', async ({ page }) => {
  // Go to the login page
  await page.goto(`${globalConfig.baseURL}ui/citizen/select-language`);
  // sign in 
  await page.getByRole('button').click();
  // Enter mobile number
  await page.getByRole('textbox').fill(globalConfig.citizenUsername);
  ...
  await page.locator('input[name="mobileNumber"]').fill(globalConfig.litigantUsername);
  ...
  await page.locator('input[name="complainantAge"]').fill(globalConfig.complainantAge);
  ...
  await page.locator('input[name="complainantAge"]').nth(1).fill(globalConfig.complainantAge);
  ...
  await page.locator('input[name="respondentFirstName"]').fill(globalConfig.respondentFirstName);
  await page.locator('div').filter({ hasText: /^Pincode$/ }).getByRole('textbox').fill(globalConfig.respondentPincode);
  await page.locator('div').filter({ hasText: /^State$/ }).getByRole('textbox').fill(globalConfig.respondentState);
  await page.locator('div').filter({ hasText: /^District$/ }).getByRole('textbox').fill(globalConfig.respondentDistrict);
  await page.locator('div').filter({ hasText: /^City \/ town$/ }).getByRole('textbox').fill(globalConfig.respondentCity);
  await page.locator('div').filter({ hasText: /^Address$/ }).getByRole('textbox').fill(globalConfig.respondentAddress);
  await page.getByRole('button').filter({ hasText: 'Continue' }).click();

   // cheque details
  
    await page.locator('input[name="chequeSignatoryName"]').click();
    await page.locator('input[name="chequeSignatoryName"]').fill(globalConfig.chequeSignatoryName);
  ...
  //   await page.locator('input[name="name"]').press('Tab');
    await page.locator('input[name="payeeBankName"]').fill(globalConfig.payeeBankName);
    await page.locator('input[name="payeeBranchName"]').click();
    await page.locator('input[name="payeeBranchName"]').fill(globalConfig.payeeBranchName);
    await page.locator('input[name="chequeNumber"]').click();
    await page.locator('input[name="chequeNumber"]').fill(globalConfig.chequeNumber);
    await page.locator('input[name="issuanceDate"]').fill(globalConfig.issuanceDate);
    await page.locator('input[name="payerBankName"]').click();
    await page.locator('input[name="payerBankName"]').fill(globalConfig.payerBankName);
    await page.locator('input[name="payerBranchName"]').click();
    await page.locator('input[name="payerBranchName"]').fill(globalConfig.payerBranchName);
    await page.locator('input[name="ifsc"]').click();
    await page.locator('input[name="ifsc"]').fill(globalConfig.ifsc);
    await page.locator('#validationCustom01').click();
    await page.locator('#validationCustom01').fill(globalConfig.chequeAmount);
    ...
    await page.getByText(globalConfig.policeStation).click();
    await page.locator('input[name="depositDate"]').fill(globalConfig.depositDate);
    await page.locator('div').filter({ hasText: /^\*Reason for the return of cheque$/ }).getByRole('textbox').click();
    await page.locator('div').filter({ hasText: /^\*Reason for the return of cheque$/ }).getByRole('textbox').fill(globalConfig.reasonForReturnOfCheque);
  ...
  await page.locator('input[type="file"]').last().setInputFiles(filePath); // Change index as needed
   await page.getByRole('button').filter({ hasText: 'Continue' }).click();
    ...
    await page.locator('input[name="liabilityNature"]').fill(globalConfig.liabilityNature);
    await page.locator('div').filter({ hasText: /^Full Liability$/ }).getByRole('radio').click();
    await page.getByRole('button').filter({ hasText: 'Continue' }).click();
  
    // legal demand notice details
  
    await page.locator('input[name="dateOfDispatch"]').fill(globalConfig.dateOfDispatch);
    await page.locator('input[type="file"]').first().setInputFiles(filePath); 
    await page.locator('input[type="file"]').nth(2).setInputFiles(filePath); 
    await page.locator('input[name="dateOfService"]').fill(globalConfig.dateOfService);
...
//    await page.getByRole('button').filter({ hasText: 'Continue' }).click();
 // Click Continue twice
  for (let i = 0; i < 2; i++) {
    ...
    await page.getByRole('textbox').first().fill(globalConfig.noOfAdvocates);
    await page.locator('input[type="file"]').first().setInputFiles(filePath);
