...
test('Show Cause Notice Order Test', async ({ page }) => {
    ...
    await page.getByText('Miscellaneous Process').click();
    await page.locator('div').filter({ hasText: /^Select Miscellaneous Process Template\*$/ }).getByRole('img').click();
    await page.getByText('Testing Automation').first().click();
    await page.locator('.select.false > .cp').click();
    await page.locator('#jk-dropdown-unique div').filter({ hasText: 'ADHUR' }).click();
    await page.locator('div:nth-child(2) > .select > .cp').click();
    await page.locator('#jk-dropdown-unique div').filter({ hasText: 'Rajesh Ch' }).click();
    await page.locator('div').filter({ hasText: /^Address$/ }).locator('svg').click();
    await page.getByRole('checkbox', { name: 'sh, sh, sh, sh, sh,' }).check();
