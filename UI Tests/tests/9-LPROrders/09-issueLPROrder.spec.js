...
test('Issue LPR Order', async ({ page }) => {
    ...
    await page.waitForTimeout(1000);
    //await page.getByRole('cell', { name: globalVars.stNumber }).click();
    await page.getByRole('cell', { name: 'ST/179/2026' }).click();
    await page.waitForTimeout(1000);
    ...
    await page.getByText('Moving Case to Long Pending').click();
    await page.getByRole('textbox', { name: 'Type here' }).click();
    await page.getByRole('textbox', { name: 'Type here' }).fill('testing');
    test.setTimeout(180000);
    ...
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(2000);
    const accessCodeElement2 = await page.locator('div.sub-details-text').filter({ hasText: 'LP/' });
    const lpNumber = await accessCodeElement2.textContent();
    console.log('LP Number:', lpNumber);
    globalVariables.lpNumber = lpNumber;
    fs.writeFileSync(globalVarsPath, JSON.stringify(globalVariables, null, 2));
    console.log('LP Number saved to global variables:', globalVariables.lpNumber);
    await page.close();
});