...
test('Fso Login Test', async ({ page }) => {
  ...
  await usernameInput.fill(globalVariables.fsoUsername);
  console.log('Entered username:', globalVariables.fsoUsername);

  // Enter password
  console.log('Entering password...');
  const passwordInput = page.locator('input[name="password"]');
  await expect(passwordInput).toBeVisible({ timeout: 10000 });
  await passwordInput.fill(globalVariables.fsoPassword);
  ...
  await page.locator('input[name="caseSearchText"]').fill(globalVariables.filingNumber);
