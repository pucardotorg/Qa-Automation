...
test("Register Case Test", async ({ page }) => {
  ...
  await page.locator(".select.undefined > .cp > path:nth-child(2)").click();
  await page.locator("#jk-dropdown-unique > div:nth-child(4)").click();
  // set the today date in the hearing date field
  ...
  await page.locator('input[name="hearingDate"]').fill(formattedDate);
  ...
  await page
    .locator(
      ".popup-module.orders-success-modal > .header-wrap > .header-end > div > svg",
    )
    .click();
