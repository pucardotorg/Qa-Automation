...
test("Evidence Test", async ({ page }) => {
  ...
  await page.waitForTimeout(2000);
  // three dot 
  await page.locator('tr:nth-child(2) > td:nth-child(8) > div > div > svg').click();

  await page.waitForTimeout(1000);
  await page.getByText('Mark as Evidence').click();

  await page.waitForLoadState("networkidle");
  await page.waitForTimeout(2000);
  // pop up opne
  await page.locator('.select.false > .cp > path:nth-child(2)').click();
  await page.locator('.cp.profile-dropdown--item').click();
  await page.locator('.label-field-pair > div > .text-input > .citizen-card-input').click();
  await page.locator('.label-field-pair > div > .text-input > .citizen-card-input').fill('1');
  await page.waitForTimeout(2000);
  await page.getByRole('button', { name: 'Proceed' }).click();
  await page.waitForLoadState("networkidle");
  await page.waitForTimeout(2000);
  await page.getByRole('button', { name: 'Send for Sign' }).click();

  await page.waitForTimeout(6000);

  console.log("Evidence marked and sent for sign successfully");
