...
test('Issue Summons', async ({ page }) => {
    ...
    console.log("Issuing Judgement Order");
    await page.waitForTimeout(1000);
    await page.locator('div').filter({ hasText: /^EditDelete$/ }).getByRole('img').first().click();
    await page.locator('#jk-dropdown-unique div').filter({ hasText: 'Judgement' }).click();
    await page.locator('div').filter({ hasText: /^Guilty$/ }).getByRole('radio').check();
    await page.locator('div').filter({ hasText: /^Finding\/Holding\*$/ }).locator('path').nth(1).click();
    await page.waitForTimeout(1000);
    await page.locator('#jk-dropdown-unique div').filter({ hasText: 'Acquitted' }).first().click();
    await page.waitForTimeout(1000);
    ...
    await page.getByRole('textbox', { name: 'Type here' }).fill('2 months of jail.');
    await page.waitForTimeout(1000);
    await page.getByRole('button').filter({ hasText: 'Confirm' }).click();
    await page.waitForTimeout(1000);
    await page.getByRole('paragraph').click();
    await page.locator('.ql-editor').fill('Judgement order');
    await page.waitForTimeout(1000);
